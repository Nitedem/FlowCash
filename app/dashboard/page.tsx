...
async function ensureWallet(userId: string, name?: string | null, email?: string | null) {
  await sql`
    ...
    ON CONFLICT (id) DO UPDATE SET full_name = COALESCE(EXCLUDED.full_name, flowcash.profiles.full_name), email = COALESCE(EXCLUDED.email, flowcash.profiles.email), updated_at = now()
  `;
  const wallets = await sql`
    INSERT INTO flowcash.wallets (user_id, currency) VALUES (${userId}, 'XAF')
  ...
  await sql`
    ...
    VALUES (${wallet.id}, 'CASH', 'XAF'), (${wallet.id}, 'AVAILABLE', 'XAF')
...
export default async function DashboardPage() {
  ...
  const transactions = (await sql`
    ...
    FROM flowcash.transactions WHERE wallet_id = ${wallet.id} ORDER BY created_at DESC LIMIT 10
  `) as TransactionRow[];
    ...
    <main className="dashboard-shell">
      <div className="dashboard-card">
        ...
        <section className="transactions-section" aria-labelledby="transactions-title">
          <div className="transactions-heading"><div><small>ACTIVITÉ FINANCIÈRE</small><h2 id="transactions-title">Historique des opérations</h2></div><span>{transactions.length} opération{transactions.length > 1 ? 's' : ''}</span></div>
          {transactions.length === 0 ? <div className="transactions-empty"><strong>Aucune opération</strong><p>Vos mouvements financiers apparaîtront ici.</p></div> : <div className="transactions-list">
            {transactions.map((transaction) => <article key={transaction.id} className={`transaction-row transaction-${transaction.status}`}>
              <div className="transaction-main"><div className="transaction-icon" aria-hidden="true">{transactionSign(transaction.type)}</div><div><strong>{transaction.description || transactionLabels[transaction.type] || transaction.type}</strong><small>{transactionLabels[transaction.type] || transaction.type} · {formatDate(transaction.created_at)}</small><small className="transaction-reference">Réf. {transaction.reference}</small></div></div>
              <div className="transaction-meta"><strong>{transactionSign(transaction.type)}{formatAmount(transaction.amount, transaction.currency)}</strong><span className={`status-badge status-${transaction.status}`}>{statusLabels[transaction.status] || transaction.status}</span></div>
            </article>)}
          </div>}
        </section>
      </div>
