...
export async function POST(request: Request) {
  ...
  const rows = await sql`
    INSERT INTO flowcash.payment_requests
      (requester_user_id, payer_user_id, requester_wallet_id, amount, currency, description, expires_at)
    VALUES
      (${user.id}, ${payer[0].id}, ${requesterWallet[0].id}, ${amountRaw}, ${currency}, ${description}, now() + (${expiresInHours} || ' hours')::interval)
    RETURNING id, amount::text, currency, status, description, expires_at::text, created_at::text
  `;
...
export async function GET() {
  ...
  const requests = await sql`
    SELECT id, requester_user_id, payer_user_id, amount::text, currency, description, status, expires_at::text, created_at::text
