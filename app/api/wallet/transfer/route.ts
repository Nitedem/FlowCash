...
function hashRequest(payload: { recipientWalletCode: string; amount: number; description: string | null }) {
...
export async function POST(request: Request) {
  ...
  try {
    const result = await withTransaction(async (client) => {
      ...
      const senderResult = await client.query(`SELECT id, status FROM flowcash.wallets WHERE user_id = $1 AND currency = 'XAF' LIMIT 1`, [user.id]);
      ...
      const recipientResult = await client.query(
        `SELECT p.id, p.full_name, w.id AS wallet_id, w.public_code, w.currency, w.status
         FROM flowcash.wallets w JOIN flowcash.profiles p ON p.id = w.user_id
         WHERE w.public_code = $1 AND w.currency = 'XAF' LIMIT 1`,
        [recipientWalletCode],
      );
      ...
      const metadataBase = { movementCode, source: 'internal', version: 2, recipientWalletCode };

      const senderTransaction = await client.query(
        `INSERT INTO flowcash.transactions (wallet_id, type, amount, currency, status, reference, movement_code, description, metadata)
         VALUES ($1, 'transfer', $2, 'XAF', 'completed', $3, $4, $5, $6::jsonb)
         RETURNING id, type, amount::text, currency, status, reference, movement_code, description, created_at::text`,
        [senderId, amount, movementCode, movementCode, description || `Transfert vers ${recipientWalletCode}`, JSON.stringify({ ...metadataBase, direction: 'outgoing' })],
      );
      ...
      const recipientTransaction = await client.query(
        `INSERT INTO flowcash.transactions (wallet_id, type, amount, currency, status, reference, movement_code, description, metadata)
         VALUES ($1, 'transfer', $2, 'XAF', 'completed', $3, $4, $5, $6::jsonb)
         RETURNING id, type, amount::text, currency, status, reference, movement_code, description, created_at::text`,
        [recipientId, amount, movementCode, movementCode, description || 'Transfert reçu', JSON.stringify({ ...metadataBase, direction: 'incoming', senderId: user.id })],
      );
      ...
      await client.query(`INSERT INTO flowcash.ledger_entries (transaction_id, account_id, wallet_id, direction, amount, currency) VALUES ($1, $2, $3, 'debit', $5, 'XAF'), ($1, $4, $3, 'credit', $5, 'XAF')`, [recipientTransaction.rows[0].id, recipientCash, recipientId, recipientAvailable, amount]);
      ...
      const response = {
        ...
        recipient: { name: recipient.full_name, walletCode: recipient.public_code, currency: recipient.currency },
