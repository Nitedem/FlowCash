    ...
    const result = await withTransaction(async (client) => {
      ...
      const transaction = await client.query(
        `INSERT INTO flowcash.transactions (wallet_id, type, amount, currency, status, reference, movement_code, description, metadata)
         VALUES ($1, $2, $3, 'XAF', 'completed', $4, $5, $6, $7::jsonb)
         RETURNING id, type, amount::text, currency, status, reference, movement_code, description, created_at::text`,
        [wallet.id, type, amount, movementCode, movementCode, description, JSON.stringify({ source: type === 'deposit' ? 'test' : 'internal', version: 2, movementCode })],
      );
      ...
      const cashAccount = accountByCode.get('CASH');
      const availableAccount = accountByCode.get('AVAILABLE');
      ...
      for (const [accountId, direction] of entries) {
        await client.query(`INSERT INTO flowcash.ledger_entries (transaction_id, account_id, wallet_id, direction, amount, currency) VALUES ($1, $2, $3, $4, $5, 'XAF')`, [transaction.rows[0].id, accountId, wallet.id, direction, amount]);
      }

      const updatedWallet = await client.query(`UPDATE flowcash.wallets SET balance = balance ${type === 'deposit' ? '+' : '-'} $1, updated_at = now() WHERE id = $2 RETURNING balance::text, currency, public_code`, [amount, wallet.id]);
      const response = { movementCode, transaction: transaction.rows[0], wallet: updatedWallet.rows[0] };
      await client.query(`UPDATE flowcash.idempotency_keys SET status = 'completed', response = $1::jsonb, updated_at = now() WHERE user_id = $2 AND idempotency_key = $3`, [JSON.stringify(response), user.id, idempotencyKey]);
