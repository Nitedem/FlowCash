...
export async function POST(request: Request) {
  ...
  if (type === 'deposit') {
    if (process.env.FLOWCASH_TEST_MODE !== 'true') {
      return jsonError('Test deposits are disabled', 403);
    }
    if (amount > MAX_TEST_DEPOSIT) {
      return jsonError('Test deposit limit exceeded', 400);
    }
  }
  ...
  try {
    const result = await withTransaction(async (client) => {
      ...
      const reference = `FC-${Date.now().toString(36).toUpperCase()}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
      const transaction = await client.query(
        `INSERT INTO flowcash.transactions
          (wallet_id, type, amount, currency, status, reference, description, metadata)
         VALUES ($1, $2, $3, 'XAF', 'completed', $4, $5, $6::jsonb)
        ...
        [
          ...
          reference,
          description,
          JSON.stringify({ source: type === 'deposit' ? 'test' : 'internal', version: 1 }),
        ],
      );
      ...
      if (!cashAccount || !availableAccount) {
        throw new Error('Wallet ledger accounts are not provisioned');
      }

      const entries = type === 'deposit'
        ? [
            [cashAccount, 'debit'],
            [availableAccount, 'credit'],
          ]
        : [
            [availableAccount, 'debit'],
            [cashAccount, 'credit'],
          ];
      ...
      for (const [accountId, direction] of entries) {
        await client.query(
          ...
          [transactionId, accountId, wallet.id, direction, amount],
        );
      }

      const updatedWallet = await client.query(
        `UPDATE flowcash.wallets
         SET balance = balance ${type === 'deposit' ? '+' : '-'} $1,
             updated_at = now()
         WHERE id = $2
